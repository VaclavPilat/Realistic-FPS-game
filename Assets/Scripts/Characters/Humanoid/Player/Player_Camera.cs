...
public class Player_Camera : Lockable_Script
{
    ...
    private void Rotate_Player ()
    {
        ...
        {
            horizontal_rotation += Input.GetAxis("Mouse X") * float.Parse(Config_Loader.Get("Mouse", "Sensitivity").Value) * Time.fixedDeltaTime;
            ...
            transform.Rotate(Vector3.up * Input.GetAxis("Mouse X") * float.Parse(Config_Loader.Get("Mouse", "Sensitivity").Value) * Time.fixedDeltaTime);
    }

    // ROTATING Human_Camera.Camera
    private void Rotate_Camera ()
    {
        vertical_rotation -= Input.GetAxis("Mouse Y") * float.Parse(Config_Loader.Get("Mouse", "Sensitivity").Value) * Time.fixedDeltaTime;
    ...
    {
        string output = "";
        output += "Sensitivity: " + float.Parse(Config_Loader.Get("Mouse", "Sensitivity").Value).ToString() + "\n";
